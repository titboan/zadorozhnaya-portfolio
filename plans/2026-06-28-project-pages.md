...
- [ ] **Уточнить тексты карточек** — названия проектов и описания взяты приблизительно из Figma; нужно сверить с реальными проектами дизайнера
  ...
  Заодно починить: в `src/data/projects.js` строки 157 и 193 ссылаются на файлы, у которых имя на диске = склеенный текст описания задачи вместо короткого имени (папки `flebologiya`, `personal-brand`). Найдено при проверке 2026-07-20, картинки физически на месте, но путь в коде не совпадает байт-в-байт (юникод-нормализация кириллицы) — переименовать файлы на диске в короткие имена и поправить пути.
